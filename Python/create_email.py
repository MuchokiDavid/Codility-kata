...
- The name of each person consists of two or three parts: a first name, an optional middle name, and a last name, separated by spaces. 
- Each part can include only English letters (but the last name may additionally contain hyphens). The name of the company also consists only of English letters.
...
- Additionally, if more than one person would have the same email address, differentiate their addresses by adding subsequent integers (starting from the second address and number 2) before the "@" sign. 
- For example, if there are three people who would have the address "jd@company.com", they should be assigned addresses "jd@company.com", "jd2@company.com" and "jd3@company.com" respectively.
...
emails= set()
def solution(S,C):
    if  len(S) and len(C) == 0:
        return None
    name_split= S.split(',')
    for nam in name_split:
        new_string= nam.replace(" ","").lower()
        no_hyphen= new_string.replace('-', '')
        emails.add(f'{no_hyphen}@{C}.com'.lower())


    # print(mail_split)
    # for em in mail_split:
        # emails.append(f'{em}@{C}.com'.lower())
    # if mail not in emails:
    # emails.append(mail)
    # if mail in emails:
    #     # mail= f'{mail_split}{r_int}@{C}.com'.lower()
    #     emails.append(f'{mail_split}{r_int}@{C}.com'.lower())

solution("John Doe, Peter Parker, Mary Jane Watson-Parker, James Doe, John Elvis Doe, Jane Doe, Penny Parker", "moringa")
